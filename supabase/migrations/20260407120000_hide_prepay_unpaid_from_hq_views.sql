-- HQ / platform: do not surface pre_pay orders until QRIS (or full voucher) is settled.
-- Customer lists are filtered in get-orders / get-home-data; these views align reporting.
...
WHERE NOT (c.customer_type = 'pre_pay' AND o.payment_status = 'unpaid');

COMMENT ON VIEW public.hq_vw_order_voucher_settlement IS
  'HQ: per-ledger-line voucher consumption tied to orders. Excludes pre_pay orders still unpaid (abandoned QRIS).';
...
LEFT JOIN public.customers c ON c.id = COALESCE(o.customer_id, h.customer_id)
WHERE NOT (o.id IS NOT NULL AND c.customer_type = 'pre_pay' AND o.payment_status = 'unpaid');

COMMENT ON VIEW public.hq_vw_order_midtrans_cash IS
  'HQ: Midtrans cash-in rows; join orders when order_id or midtrans_order_id matches. Rows tied to pre_pay+unpaid orders are hidden.';
