...
- 再创建 Midtrans 订单（`pop_...`），**QRIS 只收扣完水票后的差额**；**Snap 成功且订单保留后**，向 **`voucher_usage_ledger`** 写入用票流水（见 `docs/用票流水表与报表.md`）。
...
- 写入 **`orders` / `order_items`** 后，同样循环 **`customer_product_vouchers` UPSERT** 完成扣减；随后写入 **`voucher_usage_ledger`**（新订单路径）。

### 取消订单退回（补充）

**Edge Function：`cancel-order`**

- **客户端 App**：`pre_pay` 的 pending 订单无取消入口（仅锁定或去 QRIS）；**`later_pay` 的取消不在客户端操作**，由 **水站/员工端 App** 等调用同一接口或其它流程处理。
- 取消成功时：若存在 **`voucher_usage_ledger`** 正数行，则插入 **冲销行**；若为 **`pre_pay`** 且订单被取消，仍将按行目退回 **`customer_product_vouchers`**（与历史实现一致）。
...
| 用票对账/报表？ | **`voucher_usage_ledger`**（张数、金额、`branch` 快照等），详见 **`docs/用票流水表与报表.md`**。 |
...
4. **流水/对账**：**`voucher_usage_ledger`** 记录用票与冲销；计价规则见 **`docs/用票流水表与报表.md`**。
...
| 用票流水表 | `supabase/migrations/20260330120000_voucher_usage_ledger.sql`、`docs/用票流水表与报表.md` |
